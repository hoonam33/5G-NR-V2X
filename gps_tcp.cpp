...
void union_EPDCallback(const gps_union::EPD_01_03::ConstPtr& msg)
{
	
  //std::cout << std::setprecision(15) << msg->latitude << ", " << std::setprecision(15) << msg->longitude << std::endl;
	
  //std::stringstream ss;
  //ss << "[" << msg->timestamp << "]" << " latitude : " << std::setprecision(15) << msg->latitude << ", " << "longitude : " << std::setprecision(15) << msg->longitude;
  
  //ss << msg->latitude << "," << msg->longitude;
   //std::string message = ss.str();
   
  double latitude_raw;
  double longitude_raw;
  
  latitude_raw = (msg->latitude)*0.0000001;  
  longitude_raw = (msg->longitude)*0.0000001;
  ...
  basic_payload.device_id = 1;
  basic_payload.device_type = 2;
  basic_payload.timestamp = msg -> timestamp;
  basic_payload.latitude = msg -> latitude;
  basic_payload.longitude = msg -> longitude;
  basic_payload.heading = msg -> heading;
  basic_payload.speed = msg -> speed;
...
{
	ros::init(argc, argv, "gps_tcp");
	
	ros::NodeHandle nh;
	
	ros::Subscriber sub_union = nh.subscribe("union_EPD", 1000, union_EPDCallback);
